            ...
            <div class="col-md-12">
                <div class="form-area">
                    <div class="d-flex justify-content-between heading">
                        <h4 class=""><i class="fas fa-plus"></i> Add New abouts</h4>
                    </div>
                    ...
                    <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @forelse ($abouts as $key=>$about)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td><img class="border" style="height: 40px; width:50px;" src="{{ asset($about->image) }}" alt=""></td>
                                    <td>{{$about->description}}</td>
                                    <td>{{$about->date}}</td>
                                    <td>
                                        <a href="{{ route('abouts.edit',$about) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></a>
                                        <button type="submit" class="btn btn-danger btn-sm" onclick="deleteabout({{ $about->id }})"><i class="fa fa-trash"></i></button>
                                        <form id="delete-form-{{$about->id}}" action="{{route('abouts.destroy',$about)}}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
